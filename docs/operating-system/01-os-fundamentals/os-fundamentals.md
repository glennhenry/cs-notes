...
- **Client-Server Architecture** : A client-server architecture is a design model where it exists client processes and server processes. Client is the one that provide user interface and request specific services or resources from the OS. While the server is responsible for providing those services or resources by executing the requested operations and returning the results to the client processes.
